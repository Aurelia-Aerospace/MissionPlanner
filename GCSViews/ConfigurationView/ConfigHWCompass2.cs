...
namespace MissionPlanner.GCSViews.ConfigurationView
{
    public partial class ConfigHWCompass2 : MyUserControl, IActivate, IDeactivate
    {
        ...
        private void BUT_OBmagcalstart_Click(object sender, EventArgs e)
        {
            ...
            {
                this.LogError(ex);
                CustomMessageBox.Show(Strings.FailedToStartMagCalCheckThe + "\n" + ex.ToString(), Strings.ERROR);
                return;
            }
        ...
        {
            double value = 0;
            if (InputBox.Show(Strings.MagCalYaw, Strings.EnterCurrentHeadingInDegrees, ref value) == DialogResult.OK)
            {
                try
                {
